...
- `mode`: `crb-team` 스킬로 한 번 결정한다 (`"solo"` | `"team"`). `--team`/`--solo` 플래그가 있으면 그대로 사용하고, 없으면 crb-team 스킬의 모드 결정 흐름을 실행한다. 결정된 모드를 이후 mold·forge 호출 시 반드시 전달한다.
...
- 결과를 `.crb/outputs/{session_id}.md`에 저장
- 세션 초기화에서 결정된 mode를 반드시 전달한다 (mode가 `"team"`이면 `--team`, `"solo"`이면 `--solo`)

완료 후 체크포인트 저장 (`.crb/runs/{session_id}-checkpoint.json`):
```json
{"session_id":"crb-YYYYMMDD-HHMMSS","command":"smelt","completed_steps":["mold"],"next_step":"forge","context":{"mold_output":".crb/outputs/crb-YYYYMMDD-HHMMSS.md"},"saved_at":"<ISO8601>"}
```

요약 출력:
```
✅ mold 완료 — 설계 저장됨: .crb/outputs/{session_id}.md
```
...
완료 후 체크포인트 업데이트 (`.crb/runs/{session_id}-checkpoint.json`):
```json
{"session_id":"crb-YYYYMMDD-HHMMSS","command":"smelt","completed_steps":["mold","forge"],"next_step":"assay","context":{"mold_output":".crb/outputs/crb-YYYYMMDD-HHMMSS.md","forge_output":"<생성 파일 목록>"},"saved_at":"<ISO8601>"}
```
...
```
각 항목을 0~1로 평가 후 가중 합산한 뒤 아래 형식으로 출력한다:

  📊 만족도 점수: 0.61 (행동충족 0.4×0 + 제약준수 0.3×1 + 품질유지 0.3×1)
     이전: 없음 → 이번: 0.61  재시도 1/2

점수 변화에 따라 분기:
  이번 점수 < 이전 점수 → rollback 후 사용자에게 선택지:
    "자동 수정이 품질을 개선하지 못했습니다 (0.73 → 0.61). 원본 상태로 복원했습니다."
    > forge로 수동 수정
    > debug로 이슈 분석
    > smelt 종료

  이번 점수 >= 이전 점수 → forge --pipeline 재시도 (최대 2회)
    📊 만족도 점수: 0.73 (이전 0.61 → 개선됨)  재시도 2/2
```
...
```
1. smelt가 원본 입력에서 --holdout 항목 문자열을 제거한 명세를 mold에 전달
   예: "결제 API — Stripe 연동, 환불 처리 [--holdout "환불 처리"]"
       → mold에는 "결제 API — Stripe 연동"만 전달
2. forge도 holdout 항목 없는 mold 결과 기반으로 구현
3. 구현 완료 후 숨겨뒀던 항목으로 blind 테스트 실행
4. 통과율을 만족도 점수에 반영 (+25% 보너스, 나머지 항목 비중 조정)
```
...
- 세션 초기화 시 crb-team 스킬로 모드를 반드시 결정할 것 — mold·forge에 항상 결정된 mode를 전달해야 함. 전달 누락 시 자식 커맨드가 사용자에게 모드 선택을 물어 파이프라인이 중단됨
