...
**`--background` 단독 사용 시 주의**: `--auto` 없이 `--background`만 사용하면 Frame 단계에서 사용자 확인이 필요할 때 백그라운드 Task가 멈춘다. 백그라운드 실행 시에는 반드시 `--auto`를 함께 사용한다:
```
/crb:cast --background --auto <주제>
```
`--background`만 쓰고 `--auto`가 없으면 Frame 확인 시점에 사용자에게 알림을 보내고 포그라운드로 전환한다.
...
| Teammate | 모델 | 역할 |
|----------|------|------|
| Lens-A | Sonnet | 렌즈 1 분석 |
| Lens-B | Sonnet | 렌즈 2 분석 |
| Lens-C | Sonnet | 렌즈 3 분석 |
...
```json
{"timestamp":"<ISO8601>","session_id":"crb-YYYYMMDD-HHMMSS","command":"cast","mode":"team","topic":"<주제>","status":"completed","user_input":{"raw":"<원본 입력>","flags":[]},"explore":{"config":{"lens_a":"sonnet","lens_b":"sonnet","lens_c":"sonnet","critic":"sonnet"},"lenses":["<렌즈1>","<렌즈2>","<렌즈3>"]},"phases":{"frame":{"consensus":[],"tensions":[],"discussion_rounds":2},"design":{"chosen_direction":"<선택 방향>"},"challenge":{"reviewer":"critic-teammate","resolution":"<처리 방식>"}},"output_file":".crb/outputs/<session_id>.md"}
```
