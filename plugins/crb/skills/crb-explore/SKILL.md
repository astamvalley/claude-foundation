...
**`--lens` 플래그가 있을 때**: 사용자가 지정한 렌즈를 그대로 사용한다. 자동 선택 금지.

```
예: --lens "경제성,기술 복잡도,사용자 감정"
→ 렌즈: 경제성 / 기술 복잡도 / 사용자 감정
```

**`--lens` 플래그가 없을 때 (동적 선택)**: 주제를 보고 가장 적합한 3개의 렌즈를 직접 선택한다. 렌즈는 매번 주제 맥락에 맞게 새로 선택하며 고정하지 않는다.
