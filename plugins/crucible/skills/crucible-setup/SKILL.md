...
name: crucible-setup
description: >
  crucible 플러그인 환경을 설정한다. /crucible-setup을 실행하면
  Agent Teams 활성화, RTK 설치 확인, Codex CLI와 Gemini CLI 선택 설치를 단계별로 안내한다.
  crucible을 처음 사용하기 전, 또는 새 기기에서 환경을 세팅할 때 사용한다.
...
crucible 플러그인이 제대로 동작하는 데 필요한 환경을 단계별로 점검하고 세팅한다.
...
crucible은 여러 Agent를 병렬 실행하므로 RTK 없이는 토큰 소비가 급격히 늘어납니다.
...
Agent Teams는 crucible의 Explore 단계에서 독립 컨텍스트 병렬 실행을 가능하게 합니다.
```
...
- 설치됨 → `✅ Codex CLI 확인 — Challenge 단계에서 활용됩니다` 출력
- 미설치 → 아래 안내:

```
💡 Codex CLI (선택사항)
Codex가 있으면 Challenge 단계에서 더 독립적인 적대적 리뷰를 제공합니다.
...
- 설치됨 → `✅ Gemini CLI 확인 — Explore 렌즈 다양성이 향상됩니다` 출력
- 미설치 → 아래 안내:
...
- Codex/Gemini 미설치는 경고 없이 조용히 건너뜀
