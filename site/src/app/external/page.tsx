    ...
    <main>
      ...
      <section>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[11px] text-zinc-600 uppercase tracking-widest">External Skills</span>
          <span className="text-[11px] text-zinc-700 font-mono">{externalSkills.length}</span>
        </div>
        <div className="border-t border-zinc-800 mt-3">
          {externalSkills.map((skill, i) => (
            <Link key={skill.name} href={`/external/${skill.name}`}>
              <div className="group flex items-start gap-5 py-5 border-b border-zinc-800/60 hover:border-zinc-700 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] font-mono text-zinc-700 mt-1.5">{skill.author}</p>
                </div>
