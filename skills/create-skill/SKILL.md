...
**어떤 작업을 처리하는 스킬인가?**
한 가지 일관된 작업 단위여야 한다. 너무 좌으면 (단순 작업 하나) 스킬 효과가 없고,
...
스킬에서 가장 가치 있는 내용이다. 에이전트가 말해주지 않으면 틀리는 것들로 유지. 실제 작업 중 교정할 때마다 추가.

### 옵션 나열 말고 기본값을 제시한다

### 지침의 구체성은 작업의 민감도에 맞춘다

- **유연한 작업** → 왜(why)를 설명, 판단은 에이전트에게
...
패턴 예시가 필요하면 [references/patterns.md](references/patterns.md) 참고.
...
- description을 한국어로 쓰면 1024자 제한에 컴왜 걸림 — 작성 후 반드시 길이 확인
- references 파일 참조 시 "언제 읽을지" 조건 없이 막연하게 쓰는 경우 많음 — "에러 시 읽기"체럼 조건 명시
